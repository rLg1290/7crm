...
const AereoInter = () => {
  ...
  const exemploVoosRaw = [
    { cia_aparente: 'LATAM', numero_voo: 'LA1234', origem: 'GRU', destino: 'JFK', embarque: '06/06/2026 09:40', desembarque: '06/06/2026 17:10', duracao: '07:30', numero_conexoes: 0, adulto: 2890.50, taxa_embarque: 210.00, adicionais: 0, bagagem_despachada_23kg: 0, bagagem_mao_10kg: 1 },
    { cia_aparente: 'LATAM', numero_voo: 'LA1234', origem: 'GRU', destino: 'JFK', embarque: '06/06/2026 09:40', desembarque: '06/06/2026 17:10', duracao: '07:30', numero_conexoes: 0, adulto: 2890.50, taxa_embarque: 210.00, adicionais: 150.00, bagagem_despachada_23kg: 1, bagagem_mao_10kg: 1 },
    { cia_aparente: 'AZUL', numero_voo: 'AD5678', origem: 'GRU', destino: 'MIA', embarque: '06/06/2026 22:15', desembarque: '07/06/2026 06:05', duracao: '07:50', numero_conexoes: 1, adulto: 3120.00, taxa_embarque: 210.00, adicionais: 0, bagagem_despachada_23kg: 0, bagagem_mao_10kg: 1 },
    { cia_aparente: 'AZUL', numero_voo: 'AD5678', origem: 'GRU', destino: 'MIA', embarque: '06/06/2026 22:15', desembarque: '07/06/2026 06:05', duracao: '07:50', numero_conexoes: 1, adulto: 3120.00, taxa_embarque: 210.00, adicionais: 150.00, bagagem_despachada_23kg: 1, bagagem_mao_10kg: 1 },
    { cia_aparente: 'GOL', numero_voo: 'G3 9012', origem: 'CGH', destino: 'EZE', embarque: '06/06/2026 07:10', desembarque: '06/06/2026 09:30', duracao: '02:20', numero_conexoes: 0, adulto: 890.90, taxa_embarque: 120.00, adicionais: 0, bagagem_despachada_23kg: 0, bagagem_mao_10kg: 1 },
    { cia_aparente: 'GOL', numero_voo: 'G3 9012', origem: 'CGH', destino: 'EZE', embarque: '06/06/2026 07:10', desembarque: '06/06/2026 09:30', duracao: '02:20', numero_conexoes: 0, adulto: 890.90, taxa_embarque: 120.00, adicionais: 105.73, bagagem_despachada_23kg: 1, bagagem_mao_10kg: 1 }
  ]
  ...
  const mergeVoos = (items: any[]) => {
    ...
    items.forEach((r) => {
      ...
      const varianteBase = { bag_mao_qty: r.bag_mao_qty, bag_23_qty: r.bag_23_qty, adulto: r.adulto, taxa_embarque: r.taxa_embarque, adicionais: r.adicionais, preco_total: r.adulto + r.taxa_embarque + r.adicionais, tarifa: r.tarifa }
      if (!map.has(key)) map.set(key, { ...r, variantes: [varianteBase] })
      else {
        const v = map.get(key)
  ...
  const fallbackAirports = [
    ...
    { id: 'EZE', iata_code: 'EZE', name: 'Buenos Aires/Ezeiza', municipality: 'Buenos Aires', iso_country: 'AR' }
  ]
  ...
  const grupos = React.useMemo(() => {
    ...
    pagedResultados.forEach((r) => {
      ...
      if (!map[key]) { map[key] = { origem: r.origem, destino: r.destino, adulto: Number(r.adulto || 0), taxa: Number(r.taxa_embarque || 0), voos: [] as any[] } }
      map[key].voos.push(r)
    })
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Aéreo • Internacional</h1>
            ...
            <div className="flex items-center gap-2">
              <div className="inline-flex rounded-full border border-gray-300 overflow-hidden text-xs bg-white">
                <button type="button" className={`px-4 py-2 text-gray-700`} onClick={() => navigate('/aereodomestico')}>Doméstico</button>
                <button type="button" className={`px-4 py-2 border-l border-gray-300 bg-teal-600 text-white`}>Internacional</button>
              </div>
            </div>
          </div>
          <div className="bg-white border border-gray-200 rounded-full shadow-sm px-3 py-2 flex items-center gap-2">
            <div className="relative flex items-center gap-2 flex-1">
              ...
              {origemSelecionada ? (
                <button type="button" className="px-3 py-1 rounded-full bg-teal-50 border border-teal-300 text-teal-700 text-xs" onClick={() => { setOrigemSelecionada(null); setShowOrigemSugestoes(true); setTimeout(() => origemInputRef.current?.focus(), 0) }}>{origemSelecionada.iata_code} - {origemSelecionada.municipality || origemSelecionada.name}</button>
              ) : (
            ...
            <div className="relative flex items-center gap-2 flex-1">
              <MapPin className="h-4 w-4 text-teal-600" />
              {destinoSelecionada ? (
                <button type="button" className="px-3 py-1 rounded-full bg-teal-50 border border-teal-300 text-teal-700 text-xs" onClick={() => { setDestinoSelecionada(null); setShowDestinoSugestoes(true); setTimeout(() => destinoInputRef.current?.focus(), 0) }}>{destinoSelecionada.iata_code} - {destinoSelecionada.municipality || destinoSelecionada.name}</button>
              ) : (
                <input ref={destinoInputRef} type="text" name="destino" value={formData.destino} onChange={handleInputChange} placeholder="Destino" className="w-full bg-transparent text-sm placeholder-gray-400 focus:outline_none" onFocus={() => setShowDestinoSugestoes(true)} onBlur={() => setTimeout(() => setShowDestinoSugestoes(false), 150)} />
              )}
                ...
                <div className="absolute z-10 mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-xl p-3">
                  <div className="space-y-3">
                    ...
                    <div className="border-t border-gray-200 pt-3">
                      <div className="relative">
                        <button type="button" onClick={() => setShowClassDropdown(v => !v)} className="w-full text-left px-3 py-2 border border-gray-300 rounded-md text-sm">{formData.classe === 'EXECUTIVA' ? 'Classe: Executiva' : 'Classe: Econômica'}</button>
                        {showClassDropdown && (
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {pagedResultados.map((r, i) => {
                            ...
                            const breakdown = `Adulto: R$ ${Number(r.adulto).toLocaleString('pt-BR',{minimumFractionDigits:2})}\nTaxa: R$ ${Number(r.taxa_embarque).toLocaleString('pt-BR',{minimumFractionDigits:2})}\nAdicionais: R$ ${Number(v.adicionais||0).toLocaleString('pt-BR',{minimumFractionDigits:2})}`
                            return (
                              <tr key={i} className="border-b hover:bg-gray-50">
                                ...
                                <td className="py-2 pr-3 text-gray-900">{new Date(r.partida).toLocaleDateString('pt-BR')} • {new Date(r.partida).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</td>
                                <td className="py-2 pr-3 text-gray-900">{new Date(r.chegada).toLocaleDateString('pt-BR')} • {new Date(r.chegada).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</td>
                                <td className="py-2 pr-3 text-gray-900">
                                ...
                                <td className="py-2 pr-3 text-right" title={`Adulto: R$ ${Number(v.adulto ?? r.adulto).toLocaleString('pt-BR',{minimumFractionDigits:2})}\nTaxa: R$ ${Number(v.taxa_embarque ?? r.taxa_embarque).toLocaleString('pt-BR',{minimumFractionDigits:2})}\nAdicionais: R$ ${Number(v.adicionais||0).toLocaleString('pt-BR',{minimumFractionDigits:2})}`}>
