-- Script para corrigir a tabela fornecedores com lógica de empresa_id
...
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'fornecedores') THEN
        ...
        -- Se empresa_id IS NULL, aparece para todos os usuários
        -- Se empresa_id está definido, aparece apenas para usuários dessa empresa
...
-- 6. Se não há dados, inserir fornecedores de exemplo
DO $$
DECLARE
